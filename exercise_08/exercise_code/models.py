...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_size, self.hparams["nn_encoder_Layer1"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer1"], self.hparams["nn_encoder_Layer2"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer2"], self.hparams["nn_encoder_Layer3"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer3"], self.hparams["nn_encoder_Layer4"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer4"], self.hparams["nn_encoder_Layer5"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer5"], latent_dim)
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, self.hparams["nn_encoder_Layer5"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer5"], self.hparams["nn_encoder_Layer4"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer4"], self.hparams["nn_encoder_Layer3"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer3"], self.hparams["nn_encoder_Layer2"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer2"], self.hparams["nn_encoder_Layer1"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_encoder_Layer1"], output_size)
        )
...
class Autoencoder(nn.Module):
    ...
    def set_optimizer(self):
        ...
        self.optimizer = torch.optim.Adam(super().parameters(), self.hparams['learning_rate'])
...
class Classifier(nn.Module):
    ...
    def __init__(self, hparams, encoder):
        ...
        self.model = nn.Sequential(
            nn.Linear(self.encoder.latent_dim, self.hparams["nn_Classifier_Layer1"]),
            nn.ReLU(),
            nn.Linear(self.hparams["nn_Classifier_Layer1"], self.hparams["num_classes"])
        )
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
