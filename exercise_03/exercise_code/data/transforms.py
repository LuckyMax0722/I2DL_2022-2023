...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        images = self.min + (images-self._data_min)*(self.max-self.min)/(self._data_max-self._data_min) #formal
        #images = images/255 #unformal, only for Trans from 0-255 to 0-1
...
def compute_image_mean_and_std(images):
    ...
    mean = np.asarray([R_mean,G_mean,B_mean])
    std = np.asarray([R_std,G_std,B_std])
