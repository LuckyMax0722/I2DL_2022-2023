...
class MyOwnNetwork(ClassificationNet):
    ...
    def __init__(self, activation=Sigmoid(), num_layer=2,
                 input_size=3 * 32 * 32, hidden_size=100,
                 std=1e-3, num_classes=10, reg=0, init_w=0, init_b=0, **kwargs):
        """
        ...
        # Initialize random gaussian weights for all layers and zero bias
        self.num_layer = num_layer
        self.params = {'W1': np.random.normal(0,1/3072,(input_size, hidden_size)) + self.init_w,
                       'b1': np.zeros(hidden_size) + self.init_b}

        for i in range(num_layer - 2):
            self.params['W' + str(i + 2)] = np.random.normal(0,1/3072,(hidden_size,
                                                                  hidden_size)) + self.init_w
            self.params['b' + str(i + 2)] = np.zeros(hidden_size) + self.init_b
           

        self.params['W' + str(num_layer)] = np.random.normal(0,1/3072,(hidden_size,
                                                                  num_classes)) + self.init_w
        self.params['b' + str(num_layer)] = np.zeros(num_classes) + self.init_b
